...
void Digits::set(int n){
	...
	char s[12];
	sprintf(s,"%d",n);
	for(int i=0;s[i];++i)zdvigUP1E(s[i]-'0');
}
...
Digits:: operator int (){
	int r=0;
	for(int i=3;i>=0;--i)if(exponenta.size()>i){
		r*=100;
		r+=exponenta[i];
		}
	if(!znak)r=-r;
...
char Digits::zdvigUP1M(){
	char a,b,c=0;
	for(int i=mantisa.size()-1;i>=0;--i){
		char*x=&mantisa[i];
		a=*x/10;
		b=*x%10;
		*x=b*10+c;
...
char Digits::zdvigDOWN1E(){
	char a,b,c=0;
	for(int i=exponenta.size()-1;i>=0;--i){
		char*x=&exponenta[i];
		a=*x/10;
		b=*x%10;
		*x=c*10+a;
...
void Digits::plus1(){
	bool ok=1;
	for(int i=0;i<exponenta.size() && ok;++i){
		ok=0;
		char*h=&exponenta[i];
		++*h;
		if(*h>=100){*h=0;ok=1;}
...
char Digits::compare(Digits&A){
	...
	bool r;
	int i=exponenta.size()-1;
	for(;i>=0;--i)if(exponenta[i]!=A.exponenta[i]){
...
const char*Digits::cpi="3.141592653589793238462643383279502884197169399375105820974944592307816406286208998628034825342117067982148086513282306647093844609550582231725359408128";
const char*Digits::ce ="2.71828182845904523536028747135266249775724709369995957496696762772407663035354759457138217852516642742746639193200305992181741359662904357290033429526";
...
void Digits::away(int n){
	if(n>0){
		...
		for(int i=0;i<n;++i)exponenta[i]=0;
		if(t){
			char x=exponenta[n];
			exponenta[n]=(x-x%10);
			antiZero(exponenta);
